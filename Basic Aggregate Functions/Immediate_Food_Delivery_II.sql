...
select 
  round((count(if(D.order_date = D.customer_pref_delivery_date, D.order_date, null)) / count(D.order_date))* 100, 2) as immediate_percentage
from Delivery D
  inner join
  (select customer_id, min(order_date) as order_date
  from Delivery
  group by customer_id) D2 on D.customer_id = D2.customer_id 
                          and D.order_date = D2.order_date;
